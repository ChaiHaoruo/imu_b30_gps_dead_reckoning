...
Initializer::Initializer(const Eigen::Vector3d& init_I_p_Gps/*, float& init_B30_Noi*/) 
    : init_I_p_Gps_(init_I_p_Gps) /*,init_B30_Noi_(init_B30_Noi)*/{ }//初始化表达式
...
bool Initializer::AddB30DepthData(const B30DepthDataPtr B30_data_ptr, State* state) {
    if (imu_buffer_.size() < kImuDataBufferLength) {
        LOG(WARNING) << "[AddB30DepthDataData]: No enought imu data!";
        return false;
    }

    const ImuDataPtr last_imu_ptr = imu_buffer_.back();
    // TODO: synchronize all sensors.
    if (std::abs(B30_data_ptr->timestamp - last_imu_ptr->timestamp) > 0.5) {
        LOG(ERROR) << "[AddB30DepthDataData]: B30 and imu timestamps are not synchronized!";
        return false;
    }
    ...
    // So, we set yaw to zero and give it a big covariance.
    // if (!ComputeG_R_IFromImuData(&state->G_R_I)) {
    //     LOG(WARNING) << "[AddB30DepthDataData]: Failed to compute G_R_I!";
    //     return false;
    // }
