...
LocalizationWrapper::LocalizationWrapper(ros::NodeHandle& nh) {
    // Load configs.
    double acc_noise, gyro_noise, acc_bias_noise, gyro_bias_noise, B30_noise;
    nh.param("acc_noise",       acc_noise, 8.6*1e-8);
    nh.param("gyro_noise",      gyro_noise, 2.3*1e-9); /*bias_instability^2*/
    nh.param("acc_bias_noise",  acc_bias_noise, 1.25*1e-4);
    nh.param("gyro_bias_noise", gyro_bias_noise, 1.6*1e-5);/*rate_random_walk^2 = noise_density^2*/
    ...
    double x_B, y_B, z_B;
    nh.param("I_p_B30_x", x_B, 0.);
    nh.param("I_p_B30_y", y_B, 0.);
    nh.param("I_p_B30_z", z_B, 0.);
    const Eigen::Vector3d I_p_B30(x_B, y_B, z_B);
    ...
    file_B30_.open(log_folder +"/b30.csv");
    ...
    B30_sub = nh.subscribe("/depth", 10, &LocalizationWrapper::B30DepthCallback, this);
...
void LocalizationWrapper::B30DepthCallback(const sensor_msgs::FluidPressureConstPtr& B30_msg_ptr){
    ImuGpsLocalization::B30DepthDataPtr B30_data_ptr = std::make_shared<ImuGpsLocalization::B30DepthData>();
    B30_data_ptr->timestamp = B30_msg_ptr->header.stamp.toSec();
    B30_data_ptr->dep  = B30_msg_ptr->fluid_pressure;
    B30_data_ptr->var = B30_data_ptr->var;

    imu_gps_localizer_ptr_->ProcessB30DepthData(B30_data_ptr);
...
void LocalizationWrapper::LogB30(const ImuGpsLocalization::B30DepthDataPtr B30_data){
    file_B30_ << std::fixed <<std::setprecision(15) // fixed 提高精确度;setprecision 表示保留n位有效数字
              << B30_data->timestamp << ","
              << B30_data->dep << "\n";
}
